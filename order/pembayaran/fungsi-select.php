...
if (isset($_SESSION['email'])) {
	$qry = mysqli_query($conn,"SELECT * FROM customer WHERE email = '$mail'");
	$get = mysqli_fetch_assoc($qry);
}else{
	$qry = mysqli_query($conn,"SELECT * FROM customer_sementara WHERE email = '$mail'");
	$get = mysqli_fetch_assoc($qry);
}
$sql_p = mysqli_query($conn,"SELECT * FROM master_provinsi");
echo "<select name='provinsi' class='form-control' id='pro' onchange='pilihKota(this.value,\"$mail\")' required>";

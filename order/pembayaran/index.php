<?php
include_once("../../fungsi/variable.php");
include_once("$doc/koneksi.php");

if (isset($_SESSION['email'])) {
	$mail = $_SESSION['email'];
	//mengambil data customer berdasarkan session email
	$sql_cek = mysqli_query($conn, "SELECT * FROM customer WHERE email = '$mail'");
	$cek = mysqli_fetch_array($sql_cek);
}else{
	$mail = $_SESSION['x'];
	//mengambil data customer_sementara berdasarkan session email
	$sql_cek = mysqli_query($conn, "SELECT * FROM customer_sementara WHERE email = '$mail'");
	$cek = mysqli_fetch_array($sql_cek);
}

//menghitung total kuantitas pada tabel troli
$sql_cek2 = mysqli_query($conn, "SELECT SUM(kuantitas) AS jml FROM troli WHERE customer = '$mail'");
$cek2 	 = mysqli_fetch_array($sql_cek2);

if (empty($mail) || $cek2['jml']==0 || empty($cek['alamat']) || empty($cek['provinsi']) || empty($cek['kota']) || empty($cek['kecamatan']) || empty($cek['no_handphone'])) {
	echo "<script>window.location = '$host';</script>";
}else{
	//membuat cookie
	if (isset($_SESSION['email'])) {
		setcookie($_SESSION['nomor'],$_SESSION['email'],time()+(86400*7),"/");
	}else{
		setcookie("customer",$_COOKIE['bantuan'],time()+(86400*7),"/");
	}

	//mengecek troli, jika ada kuantitas yang 0 atau subtotal 0 maka akan di direct ke page troli
	$cek_troli = mysqli_query($conn, "SELECT * FROM troli WHERE customer = '$mail'");
	while ($get_troli = mysqli_fetch_array($cek_troli)) {
		if ($get_troli['kuantitas'] == 0 || $get_troli['subtotal'] == 0) {
			header("location: $host/troli/");
		}
	}
?>
<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>Pembayaran | EraTekno-Shop</title>
	<?php include_once("../../link-css.php"); ?>
	<style>
		.heading {
			border-left:solid 4px orange;
			background:#F4F4F4;
			box-sizing:border-box;
			-moz-box-sizing:border-box;
			-webkit-box-sizing:border-box;
		}

		.u-link { 
			background: #F4F4F4;
		}
		.u-link a{
			color: skyblue;
		}
		.u-link a:hover{
			color: orange;
		}

		#second_table {
			font-size: 13px;
			color: #878787;
			width: 100%;
			overflow-y: scroll;
		}
		#second_table a {
			color: #007FFF;
		}
		#second_table a:hover {
			color: #FFAA2A;
		}

		.btn-order {
			background: #eb4800;
			border: solid 1px #eb4800;
			color: white;
			border-radius: 5px;
			font-family: "Lucida Console", Monaco, monospace;
		}
		.btn-order:hover {
			background-image: #999;
			border: solid 1px #eb4800;
			color: #eb4800;
			border-radius: 5px;
			-webkit-transition: all 0.2s linear;
		    -moz-transition: all 0.2s linear;
		    -o-transition: all 0.2s linear;
		    -ms-transition: all 0.2s linear;
		    transition: all 0.2s linear;
		}

		#show-ket {display: none;}

.stepwizard-step p {
    margin-top: 10px;    
}

.stepwizard-row {
    display: table-row;
}

.stepwizard {
    display: table;     
    width: 100%;
    position: relative;
}

.stepwizard-step button[disabled] {
    opacity: 1 !important;
    filter: alpha(opacity=100) !important;
}

.stepwizard-row:before {
    top: 14px;
    bottom: 0;
    position: absolute;
    content: " ";
    width: 100%;
    height: 1px;
    background-color: #ccc;
    z-order: 0;
    
}

.stepwizard-step {    
    display: table-cell;
    text-align: center;
    position: relative;
}

.btn-circle {
  width: 30px;
  height: 30px;
  text-align: center;
  padding: 6px 0;
  font-size: 12px;
  line-height: 1.428571429;
  border-radius: 15px;
}
	</style>
</head>
<body onload="trans();">
	<div id="gototop"></div>
	
	<!-- Loading -->
	<div id="preloader" class="hidden-xs hidden-sm">
		<div id="status"></div>
	</div>
	
	<?php include_once("../../navigasi-fixed.php"); ?>
	<div id="kategory"><button id="btn-kat" class="btn btn-default btn-lg">Kategori Belanja <i class="glyphicon glyphicon-circle-arrow-down"></i></button><div id="slide-kat"><?php include_once("../../kategori.php"); ?></div>
		<?php
		include_once("../../fungsi/fungsi.php");
		breadcrumbs4("Troli","Email ID","PENGISIAN ALAMAT","PEMBAYARAN",$host,"$host/troli/","$host/order/checkout.php","$host/order/");
		?>
	</div>

	<div class="container">

		<div class="row">
			<div class="col-md-12 col-sm-12">
				<div class="well well-small" style="background:white;padding:7px;">
					<div class="stepwizard">
					    <div class="stepwizard-row">
					        <div class="stepwizard-step">
					            <button type="button" class="btn btn-default btn-circle">1</button>
					            <p>Cart</p>
					        </div>
					        <div class="stepwizard-step">
					            <button type="button" class="btn btn-default btn-circle">2</button>
					            <p>Email ID</p>
					        </div>
					        <div class="stepwizard-step">
					            <button type="button" class="btn btn-default btn-circle">3</button>
					            <p>Shipping</p>
					        </div>
					        <div class="stepwizard-step">
					            <button type="button" class="btn btn-primary btn-circle">4</button>
					            <p>Payment</p>
					        </div> 
					    </div>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<div class="col-md-8 col-sm-8">
				<div class="well well-small" style="background:white;padding:7px;">
				<?php
				include_once("pembayaran.php");
				?>
				</div>
			</div>

			<div class="col-md-4 col-sm-4">
				<div class="well well-small" style="background:white;padding:5px;width:100%;">
				<?php
				include_once("alamat_customer.php");
				include_once("detil-order.php");
				?>
				</div>
			</div>
		</div>

	</div>

	<?php include_once("../../footer.php"); ?>

<?php include_once("../../link-js.php"); ?>
<script src="<?php echo "$host/asset/js/kategori-fixed.js"; ?>"></script>
<script>
var host = "<?php echo $host; ?>";

	function ibank() {
		$.ajax({
			url: host+'/order/pembayaran/metode-bayar-ajax.php',
			type: 'POST',
			datatype: 'php',
			data: 'i=inet_b',
			success: function(hasil){
				$('#inet_bank').html(hasil);
			},
		});
		$("#inet_bank").css("display","block");
		$("#visa-mcard").css("display","none");
		$("#trans_b").css("display","none");
		$("#bg-ibank").css("background","#F4F4F4");
		$("#bg-kredit").css("background","white");
		$("#bg-trans").css("background","white");
	}

	function visa() {
		$.ajax({
			url: host+'/order/pembayaran/metode-bayar-ajax.php',
			type: 'POST',
			datatype: 'php',
			data: 'v=visa',
			success: function(hasil){
				$('#visa-mcard').html(hasil);
			},
		});
		$("#visa-mcard").css("display","block");
		$("#inet_bank").css("display","none");
		$("#trans_b").css("display","none");
		$("#bg-kredit").css("background","#F4F4F4");
		$("#bg-ibank").css("background","white");
		$("#bg-trans").css("background","white");
	}

	function trans() {
		$.ajax({
			url: host+'/order/pembayaran/metode-bayar-ajax.php',
			type: 'POST',
			datatype: 'php',
			data: 't=transfer',
			success: function(hasil){
				$('#trans_b').html(hasil);
			},
		});
		$("#trans_b").css("display","block");
		$("#inet_bank").css("display","none");
		$("#visa-mcard").css("display","none");
		$("#bg-trans").css("background","#F4F4F4");
		$("#bg-ibank").css("background","white");
		$("#bg-kredit").css("background","white");
		$("#pesanan2, .p").remove();
		$("#pesanan").show();
	}

	function pilihKota(c,s) {
		$.ajax({
			url: host+'/order/pembayaran/select-alamat-ajax.php',
			type: 'POST',
			datatype: 'php',
			data: 'pick_city='+c+'&sesi='+s,
			success: function(hasil){
				$('#city').html(hasil);
			},
		});
		$("#ko").remove();
		$("#kec").remove();
		$("#city").html("Loading...");
	}

	function pilihKecamatan(k,s) {
		$.ajax({
			url: host+'/order/pembayaran/select-alamat-ajax.php',
			type: 'POST',
			datatype: 'php',
			data: 'pick_kcm='+k+'&sesi2='+s,
			success: function(hasil){
				$('#kcmt').html(hasil);
			},
		});
		$("#kec").remove();
		$("#hpus_kec").remove();
		$("#kcmt").html("Loading...");
	}

    $("#i_bank").click(function() {
    	$("#pesanan2, .p").remove();
        $("#pesanan").hide();
        $("#tombol").append('<button type="button" class="btn btn-default btn-lg" id="pesanan2" disabled="disabled" style="pointer:not-allowed;">KONFIRMASI PESANAN</button> <p class="p" style="color:red;padding:6px;">Maaf, saat ini hanya bisa melakukan metode pembayaran Transfer.</p>');
    });
    $("#kredit").click(function() {
    	$("#pesanan2, .p").remove();
        $("#pesanan").hide();
        $("#tombol").append('<button type="button" class="btn btn-default btn-lg" id="pesanan2" disabled="disabled" style="pointer:not-allowed;">KONFIRMASI PESANAN</button> <p style="color:red;padding:6px;" class="p">Maaf, saat ini hanya bisa melakukan metode pembayaran Transfer.</p>');
    });

    function klik_trans() {
    	$("#show-ket").css("display","block");
    }
    function hide_ket() {
    	$("#show-ket").css("display","none");
    }

</script>
</body>
</html>
<?php
}
?>
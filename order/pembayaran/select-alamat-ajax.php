...
if (isset($_POST['pick_city'])) {
	...
	if (isset($_SESSION['email'])) {
		...
		$qry = mysqli_query($conn, "SELECT * FROM customer_sementara WHERE email = '".$_POST['sesi']."'");
		$get = mysqli_fetch_assoc($qry);
	}
...
if (isset($_POST['pick_kcm'])) {
	...
	if (isset($_SESSION['email'])) {
		...
		$qry2 = mysqli_query($conn, "SELECT * FROM customer_sementara WHERE email = '".$_POST['sesi2']."'");
		$get2 = mysqli_fetch_assoc($qry2);
	}
